...
새로고침되며 페이지를 이동시키는 방식  
html 문서, 데이터 등을 받아 조립해야 함
...
> 1️⃣ 이전의 방식 : SSR -> CSR
> 서버에서 HTML을 통으로 내려주고, 클라이언트에서는 제이쿼리 등으로 DOM만 제어  
> -> 이제 그리는 것도 클라이언트가 제어하겠다 => CSR

> 2️⃣ CSR -> SSR
> 서버에서 HTML을 통으로 내려주고, 클라이언트에서 여기에 생명력을 주입하자   
> 그 뒤로는 리액트의 CSR과 동일하게 쓸 수 있지 않을까?
...
읽어오는 시간 소요
-> 자바스크립트 파일을 불러와야 하기 때문 `<script src="bunble.js" />`
다운로드받고, 분석하고, 실행해야 함

앱의 사이즈가 커졌다 => js 파일의 용량이 커졌다 => 파일 다운 시간이 오래 소요 

#### 해결 방법

1. 웹팩 등의 번들러 -> chunk를 나누기 -> 처음에 가장 중요한 부분부터 불러오고, 덜 중요한 부분은 defer, async 등으로 처리
2. SSR로 회귀 

#### React로 CSR을 사용하는 이유

리액트는 싱글페이지 어플리케이션을 쉽게 만들 수 있는 라이브러리  
리액트의 VDOM을 이용해 효율적으로 변화를 업데이트할 수 있음  
가상 돔과 실제 돔을 비교해 실질적인 변화가 있는 부분을 교체 -> 선언적 프로그래밍이 가능  

<br>

### SSG

> 서버에서 HTML을 통으로 내려주고, 클라이언트에서는 아무것도 하지 않음 (옛날과 동일 방식)
